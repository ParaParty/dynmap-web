componentconstructors['coord'] = function(dynmap, configuration) {
	...
	dynmap.map.on('mousemove', function(mevent) {
		...
		if (dynmap.lastMouseStatus==0) {
			$(".coord-control").removeClass("fadeout")
			$(".coord-control").addClass("fadein")
		}
		dynmap.lastMouseStatus=1;
	});
	dynmap.map.on('mouseout', function(mevent) {
		...
		if (dynmap.lastMouseStatus==1) {
			$(".coord-control").removeClass("fadein")
			$(".coord-control").addClass("fadeout")
		}
		dynmap.lastMouseStatus=0;
	});
};
